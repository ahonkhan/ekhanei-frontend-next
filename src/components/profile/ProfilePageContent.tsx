...
export const ProfilePageContent: React.FC<ProfilePageContentProps> = ({ initialTab = 'orders' }) => {
  ...
  useEffect(() => {
    ...
    if (activeUser) {
      ...
      setProfileWhatsapp(activeUser.whatsapp_number || (activeUser as any).whatsapp || '');
  ...
  const menuItems = [
    ...
    { id: 'theme', label: 'App Theme (থিম)', icon: Palette },
          ...
          <main className="lg:col-span-8 xl:col-span-9 space-y-5">
                              ...
                              {(ordStatus === 'completed' || ordStatus === 'delivered') && (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    setReviewTarget({
                                      productId: String(firstItem?.product_id || firstItem?.id || ord.id),
                                      productName: firstItem?.product_name || firstItem?.name || `Order #${ord.order_number || ord.id}`,
                                      productImage: firstItem?.image || firstItem?.product_image,
                                      orderId: String(ord.id),
                                    });
                                  }}
                                  className="px-3.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-800 text-xs font-extrabold rounded-xl transition flex items-center gap-1.5 cursor-pointer shadow-xs border border-amber-200"
                                >
                ...
                <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-4 max-w-2xl">
                  ...
                  <form onSubmit={handleProfileSave} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-700">WhatsApp Number (হোয়াটসঅ্যাপ নম্বর)</label>
                        <input
                          type="tel"
            ...
            {activeTab === 'settings' && (
              <div className="space-y-5">
                ...
                {/* Theme Preferences Block in Settings */}
                <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-4 max-w-2xl">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h3 className="font-extrabold text-sm text-slate-800">App Theme (অ্যাপ থিম)</h3>
                        <p className="text-[11px] text-slate-500">Choose your preferred color theme across the application</p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={toggleThemeModal}
                      className="px-3 py-1.5 rounded-xl bg-theme-primary text-white text-xs font-bold shadow-xs hover:bg-theme-primary-hover transition cursor-pointer flex items-center gap-1"
                    >
                      <Palette className="w-3.5 h-3.5" />
                      <span>Modal Switcher</span>
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5">
                    {themes.map((t) => {
                      ...
                      return (
                        ...
                        >
                          <div className="w-8 h-8 rounded-lg overflow-hidden flex shadow-2xs border border-slate-200">
                            <div className="w-1/2 h-full" style={{ backgroundColor: t.previewColors[0] }} />
                            <div className="w-1/2 h-full" style={{ backgroundColor: t.previewColors[1] }} />
                          </div>
                          <span className="text-[11px] font-bold text-slate-800 truncate w-full">{t.nameBn}</span>
                        </button>
                      );
                    })}
                  </div>
              ...
              <div className="space-y-5">
                <div>
                  <div className="flex items-center gap-2.5">
                    ...
                    <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
                      {siteSettings?.helpline_title || 'Customer Helpline & Support'}
                    </h1>
                  </div>
                  <p className="text-xs sm:text-sm font-medium text-slate-500 mt-0.5">
                    Need help or have questions? Reach out to us via call, email, or social media channels below
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {/* Helpline Phone */}
                  <a
                    href={`tel:${siteSettings?.helpline_phone || siteSettings?.contact_phone || '+8801969901212'}`}
                    className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs hover:border-emerald-500 hover:shadow-md transition text-center space-y-2 group block"
                  >
                    ...
                    <h4 className="font-extrabold text-sm text-slate-800">{siteSettings?.helpline_title || 'Helpline Support'}</h4>
                    <p className="text-xs text-emerald-700 font-extrabold">{siteSettings?.helpline_phone || siteSettings?.contact_phone || '+8801969901212'}</p>
                    <span className="text-[10px] text-slate-400 font-bold block">Click to Call Now</span>
                  </a>

                  {/* Support Email */}
                  <a
                    href={`mailto:${siteSettings?.support_email || 'support@govaly.com.bd'}`}
                    className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs hover:border-emerald-500 hover:shadow-md transition text-center space-y-2 group block"
                  >
                    ...
                    <h4 className="font-extrabold text-sm text-slate-800">Email Support</h4>
                    <p className="text-xs text-blue-700 font-bold truncate">{siteSettings?.support_email || 'support@govaly.com.bd'}</p>
                    <span className="text-[10px] text-slate-400 font-bold block">Send us an Email</span>
                  </a>
                  ...
                  {siteSettings?.whatsapp_number && (
                    ...
                    >
                      ...
                      <h4 className="font-extrabold text-sm text-slate-800">WhatsApp Support</h4>
                      <p className="text-xs text-emerald-700 font-bold">{siteSettings.whatsapp_number}</p>
                      <span className="text-[10px] text-slate-400 font-bold block">Chat on WhatsApp</span>
                    </a>
                  )}
                  ...
                  {siteSettings?.facebook_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs hover:border-blue-500 hover:shadow-md transition text-center space-y-2 group block"
                    >
                      ...
                      <h4 className="font-extrabold text-sm text-slate-800">Facebook Page</h4>
                      <p className="text-xs text-blue-600 font-bold">Follow Us on Facebook</p>
                      <span className="text-[10px] text-slate-400 font-bold block">Visit Official Page</span>
                    </a>
                  )}
                  ...
                  {siteSettings?.youtube_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs hover:border-red-500 hover:shadow-md transition text-center space-y-2 group block"
                    >
                      <div className="w-12 h-12 rounded-2xl bg-rose-100 text-rose-600 flex items-center justify-center mx-auto group-hover:scale-110 transition-transform">
                        <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24">
                          <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                        </svg>
                      </div>
                      <h4 className="font-extrabold text-sm text-slate-800">YouTube Channel</h4>
                      <p className="text-xs text-rose-600 font-bold">Watch Videos & Guides</p>
                      <span className="text-[10px] text-slate-400 font-bold block">Subscribe Channel</span>
                    </a>
                  )}
                  ...
                  {siteSettings?.instagram_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs hover:border-pink-500 hover:shadow-md transition text-center space-y-2 group block"
                    >
                      <div className="w-12 h-12 rounded-2xl bg-pink-100 text-pink-600 flex items-center justify-center mx-auto group-hover:scale-110 transition-transform">
                        <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24">
                      ...
                      <h4 className="font-extrabold text-sm text-slate-800">Instagram</h4>
                      <p className="text-xs text-pink-600 font-bold">Follow @Instagram</p>
                      <span className="text-[10px] text-slate-400 font-bold block">Visit Instagram Profile</span>
                    </a>
                  )}

                  {/* Physical Address */}
                  {siteSettings?.company_address && (
                    <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs text-center space-y-2 col-span-1 sm:col-span-2 lg:col-span-3">
                      <div className="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto">
                        <MapPin className="w-6 h-6" />
                      </div>
                      <h4 className="font-extrabold text-sm text-slate-800">Office & Hub Location</h4>
                      <p className="text-xs text-slate-700 font-bold max-w-lg mx-auto">{siteSettings.company_address}</p>
                      <span className="text-[10px] text-slate-400 font-bold block">Official Business Address</span>
                    </div>
                  )}
                </div>
              </div>
