...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <>
      {/* Floating Top-Right Google One Tap Prompt Widget (as requested in screenshot) */}
      {showGooglePrompt && (
        <div className="fixed top-4 right-4 z-[999999] w-[320px] max-w-[90vw] bg-[#18191a] text-white rounded-2xl p-3.5 shadow-2xl border border-slate-700/80 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center justify-between gap-2.5">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <p className="text-xs font-semibold text-white leading-snug">
                Google এর মাধ্যমে <span className="font-bold">ekhanei.com.bd</span> এ প্রবেশ করুন
              </p>
            </div>
            <button
              type="button"
              onClick={() => setShowGooglePrompt(false)}
              className="text-slate-400 hover:text-white p-1 rounded-full hover:bg-white/10 transition cursor-pointer shrink-0"
              title="Close"
            >
              <X className="w-4 h-4" />
            </button>
          ...
          <button
            ...
            className="w-full mt-3 py-2 px-4 bg-[#1a73e8] hover:bg-[#1557b0] active:scale-[0.98] text-white font-extrabold text-xs rounded-full shadow-md transition flex items-center justify-center cursor-pointer"
          >
            চালিয়ে যান
          </button>
        </div>
